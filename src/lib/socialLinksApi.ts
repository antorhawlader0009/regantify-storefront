 ...
 * social-link fields off it — a page with its own server-fetched
 * StorefrontInfo (home, product, cart, checkout) should pass those
 * fields straight through as a prop instead of calling this, since
 * that's already the same data with no extra round trip. Falls back to
 * an empty object rather than throwing: a footer with no logo/social
 * icons is a fine degraded state, not worth blocking the page over.
 */
